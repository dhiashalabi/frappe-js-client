...
describe('createFrappeClient', () => {
    it('defaults to apiVersion 2 and a 30s timeout', () => {
        const client = createFrappeClient({ frappeVersion: 16, url: 'https://example.com' })
    ...
    it('is immutable — withAuth returns a new client and leaves the original untouched', () => {
        const original = createFrappeClient({ frappeVersion: 16, url: 'https://example.com', auth: anonymousAuth() })
    ...
    it('withHeaders merges rather than replaces', () => {
        const original = createFrappeClient({ frappeVersion: 16, url: 'https://example.com', headers: { 'X-A': '1' } })
    ...
    it('withMiddleware appends to the existing pipeline', () => {
        ...
        const original = createFrappeClient({ frappeVersion: 16, url: 'https://example.com', middleware: [mw1] })
    ...
    it('builds a v1 client and exposes __internal for frappe-js-client/extended', () => {
        const client = createFrappeClient({ frappeVersion: 16, url: 'https://example.com', apiVersion: 1 })
    ...
    it('captures Set-Cookie onto cookieAuth after a response, via AuthStrategy.onResponse', async () => {
        ...
        const client = createFrappeClient({ frappeVersion: 16, url: 'https://example.com', auth })
