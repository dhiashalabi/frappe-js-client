...
import { FRAPPE_TEST_ADMIN_PASSWORD, FRAPPE_TEST_FRAPPE_VERSION, FRAPPE_TEST_URL } from './setup'

describe.skipIf(FRAPPE_TEST_FRAPPE_VERSION === 14)('integration: v1 (classic) vs v2 behavioral parity', () => {
    it('getDoc returns the same document shape on v1 and v2', async () => {
        const v1 = createFrappeClient({ frappeVersion: FRAPPE_TEST_FRAPPE_VERSION, url: FRAPPE_TEST_URL, apiVersion: 1, auth: cookieAuth() })
        const v2 = createFrappeClient({ frappeVersion: FRAPPE_TEST_FRAPPE_VERSION, url: FRAPPE_TEST_URL, apiVersion: 2, auth: cookieAuth() })
    ...
    it('v1 rejects v2-only features with FeatureNotSupportedError before ever hitting the network', async () => {
        const v1 = createFrappeClient({ frappeVersion: FRAPPE_TEST_FRAPPE_VERSION, url: FRAPPE_TEST_URL, apiVersion: 1 })
