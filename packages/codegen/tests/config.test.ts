...
describe('mergeConfig', () => {
    const envKeys = ['FRAPPE_URL', 'FRAPPE_API_KEY', 'FRAPPE_API_SECRET'] as const
    ...
    it('uses FRAPPE_URL when url is not in file or flags', () => {
        setEnv('FRAPPE_URL', 'https://from-env.example')
        expect(mergeConfig(undefined, {}).url).toBe('https://from-env.example')
        setEnv('FRAPPE_URL', '')
        expect(mergeConfig(undefined, {}).url).toBeUndefined()
    })
})
